...
在 AI Agent 执行任务时，上下文会不可避免地随着交互历史而增长——工具调用、中间步骤、报错信息不断堆积，Token 消耗飙升，模型性能下降。当增长过快，冗长的上下文很快变成噪音。

常见的应对方式是对过去的上下文进行压缩。但这需要回答两个棘手的问题：
1. 如何压缩？
2. 压缩哪些内容？

要很好地回答这两个问题极其困难：基于 Token 删除的方法（如滑动窗口）只能无差别丢弃内容，不知道哪里是推理链的关键节点；基于 LLM 摘要的方法，本质上是让模型猜“未来会用到什么”。而任何猜错，都是永久性的信息丢失。

于是我换了一个思路：

>将复杂任务隔离在子 Agent 中，让它拥有独立的上下文，完成后只返回执行摘要。

子 Agent 内部的完整上下文并未消失——它被完整保留在自己的事件流中，随时可以审计和重放。主 Agent 的上下文只接收摘要，膨胀速度被大幅降低。这相当于用“负载均衡”替代了“压缩”，用结构化的任务委托回答了那两个棘手的问题。
